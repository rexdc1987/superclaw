...
class XiaohongshuAdapter(BasePlatformAdapter):
    ...
    async def login(self, account) -> bool:
        ...
        try:
            login_btn = await page.query_selector('[class*="login"], [class*="sign-in"]')
            return login_btn is None  # No login button means logged in
    ...
    async def search_keyword(self, keyword: str, video_count: int = 10) -> List[Dict]:
        ...
        try:
            ...
            cards = await page.query_selector_all('[class*="note-item"], [class*="search-result"]')
    ...
    async def get_comments(self, video_url: str, count: int = 50) -> List[Dict]:
        ...
        try:
            for _ in range(max(1, count // 10)):
                ...
                await asyncio.sleep(1)
            els = await page.query_selector_all('[class*="comment-item"], [class*="comment-inner"]')
            for el in els[:count]:
                try:
                    text_el = await el.query_selector('[class*="content"], [class*="note-text"]')
    ...
    async def post_comment(self, video_url: str, content: str) -> bool:
        ...
        try:
            ...
            inp = await page.query_selector('[class*="comment-input"], textarea')
            if inp:
                await inp.type(content)
                btn = await page.query_selector('[class*="submit"], button:has-text("发送")')
                if btn:
                    await btn.click()
                    return True
            return False
    ...
    async def like_comment(self, comment_url: str) -> bool:
        ...
        if not page: return False
        try:
            btn = await page.query_selector('[class*="like-btn"], [class*="like-icon"]')
            if btn:
                await btn.click()
                return True
            return False
        except Exception as e:
            logger.error(f"like_comment failed: {e}")
            return False
